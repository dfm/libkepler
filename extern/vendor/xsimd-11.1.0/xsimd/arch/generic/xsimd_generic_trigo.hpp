...
template <class A, class T>
inline batch<std::complex<T>, A> acos(const batch<std::complex<T>, A>& z,
...
template <class A, class T>
inline batch<std::complex<T>, A> acosh(const batch<std::complex<T>, A>& z,
                                       requires_arch<generic>) noexcept {
  using batch_type = batch<std::complex<T>, A>;
  batch_type w = acos(z);
...
template <class A, class T>
inline batch<std::complex<T>, A> asin(const batch<std::complex<T>, A>& z,
                                      requires_arch<generic>) noexcept {
  using batch_type = batch<std::complex<T>, A>;
  using real_batch = typename batch_type::real_batch;
  real_batch x = z.real();
  ...
  return select(
      y == real_batch(0.),
      select(fabs(x) > real_batch(1.), batch_type(constants::pio2<real_batch>(), real_batch(0.)),
...
inline batch<T, A> average(const batch<T, A>& x1, const batch<T, A>& x2) noexcept {
  return (x1 & x2) + ((x1 ^ x2) >> 1);
}

template <class A, class T>
inline batch<T, A> averagef(const batch<T, A>& x1, const batch<T, A>& x2) noexcept {
  using batch_type = batch<T, A>;
...
template <class A, class T>
inline batch<std::complex<T>, A> asinh(const batch<std::complex<T>, A>& z,
                                       requires_arch<generic>) noexcept {
  using batch_type = batch<std::complex<T>, A>;
...
namespace detail {
template <class A>
static inline batch<float, A> kernel_atan(const batch<float, A>& x,
...
template <class A>
static inline batch<double, A> kernel_atan(const batch<double, A>& x,
...
template <class A, class T>
inline batch<std::complex<T>, A> atan(const batch<std::complex<T>, A>& z,
                                      requires_arch<generic>) noexcept {
  using batch_type = batch<std::complex<T>, A>;
  using real_batch = typename batch_type::real_batch;
  real_batch x = z.real();
...
template <class A, class T>
inline batch<std::complex<T>, A> atanh(const batch<std::complex<T>, A>& z,
                                       requires_arch<generic>) noexcept {
  using batch_type = batch<std::complex<T>, A>;
...
// atan2
template <class A, class T>
inline batch<T, A> atan2(batch<T, A> const& self, batch<T, A> const& other,
                         requires_arch<generic>) noexcept {
  using batch_type = batch<T, A>;
...
template <class A>
static inline batch<double, A> sin_eval(const batch<double, A>& z,
...
template <class A, class T>
inline batch<std::complex<T>, A> cos(batch<std::complex<T>, A> const& z,
                                     requires_arch<generic>) noexcept {
...
template <class A, class T>
inline batch<std::complex<T>, A> cosh(const batch<std::complex<T>, A>& z,
                                      requires_arch<generic>) noexcept {
  auto x = z.real();
  auto y = z.imag();
...
template <class A, class T>
inline batch<std::complex<T>, A> sin(batch<std::complex<T>, A> const& z,
                                     requires_arch<generic>) noexcept {
...
// sincos
template <class A, class T>
inline std::pair<batch<T, A>, batch<T, A>> sincos(batch<T, A> const& self,
                                                  requires_arch<generic>) noexcept {
  using batch_type = batch<T, A>;
  const batch_type x = abs(self);
  batch_type xr = constants::nan<batch_type>();
...
template <class A, class T>
inline std::pair<batch<std::complex<T>, A>, batch<std::complex<T>, A>> sincos(
    batch<std::complex<T>, A> const& z, requires_arch<generic>) noexcept {
  using batch_type = batch<std::complex<T>, A>;
  using real_batch = typename batch_type::real_batch;
...
template <class A, class T>
inline batch<std::complex<T>, A> sinh(const batch<std::complex<T>, A>& z,
                                      requires_arch<generic>) noexcept {
  auto x = z.real();
  auto y = z.imag();
...
template <class A, class T>
inline batch<std::complex<T>, A> tan(batch<std::complex<T>, A> const& z,
                                     requires_arch<generic>) noexcept {
  using batch_type = batch<std::complex<T>, A>;
  using real_batch = typename batch_type::real_batch;
...
inline batch<T, A> tanh(batch<T, A> const& self, requires_arch<generic>) noexcept {
  ...
  if (any(test)) {
    ...
    if (all(test)) return z ^ bts;
  }
  batch_type r = fma(batch_type(-2.), one / (one + exp(x + x)), one);
  return select(test, z, r) ^ bts;
}
template <class A, class T>
inline batch<std::complex<T>, A> tanh(const batch<std::complex<T>, A>& z,
                                      requires_arch<generic>) noexcept {
