...
template <class T>
svbool_t sve_ptrue() noexcept {
  return sve_ptrue_impl(index<sizeof(T)>{});
}
...
template <class T>
inline uint64_t sve_pcount(svbool_t p) noexcept {
...
inline batch<std::complex<T>, A> load_complex_aligned(std::complex<T> const* mem,
...
template <class A, class T, detail::sve_enable_floating_point_t<T> = 0>
inline batch<std::complex<T>, A> load_complex_unaligned(std::complex<T> const* mem,
                                                        convert<std::complex<T>>,
                                                        requires_arch<sve>) noexcept {
...
inline void scatter(batch<T, A> const& src, T* dst, batch<U, A> const& index,
...
template <class A>
inline batch<float, A> bitwise_and(batch<float, A> const& lhs, batch<float, A> const& rhs,
                                   requires_arch<sve>) noexcept {
...
template <class A>
inline batch<float, A> bitwise_andnot(batch<float, A> const& lhs, batch<float, A> const& rhs,
                                      requires_arch<sve>) noexcept {
  const auto lhs_bits = svreinterpret_u32(lhs);
  const auto rhs_bits = svreinterpret_u32(rhs);
...
template <class A>
inline batch<float, A> bitwise_or(batch<float, A> const& lhs, batch<float, A> const& rhs,
                                  requires_arch<sve>) noexcept {
  const auto lhs_bits = svreinterpret_u32(lhs);
  const auto rhs_bits = svreinterpret_u32(rhs);
...
template <class A>
inline batch<float, A> bitwise_xor(batch<float, A> const& lhs, batch<float, A> const& rhs,
                                   requires_arch<sve>) noexcept {
  const auto lhs_bits = svreinterpret_u32(lhs);
  const auto rhs_bits = svreinterpret_u32(rhs);
...
// neq
template <class A, class T, detail::sve_enable_all_t<T> = 0>
inline batch_bool<T, A> neq(batch<T, A> const& lhs, batch<T, A> const& rhs,
                            requires_arch<sve>) noexcept {
...
inline batch<T, A> swizzle(batch<T, A> const& arg, batch_constant<batch<I, A>, idx...>,
                           requires_arch<sve>) noexcept {
...
inline batch<T, A> sve_extract_pair(batch<T, A> const&, batch<T, A> const& /*rhs*/, std::size_t,
...
inline batch<T, A> sve_extract_pair(batch<T, A> const& lhs, batch<T, A> const& rhs, std::size_t n,
...
inline batch<T, A> sve_extract_pair_impl(batch<T, A> const& lhs, batch<T, A> const& rhs,
                                         std::size_t n,
...
inline batch<float, A> fast_cast(batch<T, A> const& arg, batch<float, A> const&,
                                 requires_arch<sve>) noexcept {
  return svcvt_f32_x(detail::sve_ptrue<T>(), arg);
}

template <class A, class T, detail::enable_sized_integral_t<T, 8> = 0>
inline batch<double, A> fast_cast(batch<T, A> const& arg, batch<double, A> const&,
                                  requires_arch<sve>) noexcept {
  return svcvt_f64_x(detail::sve_ptrue<T>(), arg);
}

template <class A>
inline batch<int32_t, A> fast_cast(batch<float, A> const& arg, batch<int32_t, A> const&,
                                   requires_arch<sve>) noexcept {
  return svcvt_s32_x(detail::sve_ptrue<float>(), arg);
}

template <class A>
inline batch<uint32_t, A> fast_cast(batch<float, A> const& arg, batch<uint32_t, A> const&,
                                    requires_arch<sve>) noexcept {
  return svcvt_u32_x(detail::sve_ptrue<float>(), arg);
}

template <class A>
inline batch<int64_t, A> fast_cast(batch<double, A> const& arg, batch<int64_t, A> const&,
                                   requires_arch<sve>) noexcept {
  return svcvt_s64_x(detail::sve_ptrue<double>(), arg);
}

template <class A>
inline batch<uint64_t, A> fast_cast(batch<double, A> const& arg, batch<uint64_t, A> const&,
                                    requires_arch<sve>) noexcept {
...
template <class A, class T, detail::sve_enable_all_t<T> = 0>
inline batch<float, A> bitwise_cast(batch<T, A> const& arg, batch<float, A> const&,
                                    requires_arch<sve>) noexcept {
...
inline batch_bool<T_out, A> batch_bool_cast(batch_bool<T_in, A> const& arg,
...
// nearbyint_as_int
template <class A>
inline batch<int32_t, A> nearbyint_as_int(batch<float, A> const& arg,
                                          requires_arch<sve>) noexcept {
...
template <class A>
inline batch<int64_t, A> nearbyint_as_int(batch<double, A> const& arg,
                                          requires_arch<sve>) noexcept {
...
// ldexp
template <class A, class T, detail::sve_enable_floating_point_t<T> = 0>
inline batch<T, A> ldexp(const batch<T, A>& x, const batch<as_integer_t<T>, A>& exp,
                         requires_arch<sve>) noexcept {
