...
inline batch<float, A> fast_cast(batch<uint32_t, A> const& v, batch<float, A> const&,
  ...
  __m256i v_lo =
      bitwise_and(batch<uint32_t, A>(v),
                  batch<uint32_t, A>(msk_lo)); /* extract the 16 lowest significant bits of self */
  __m256i v_hi =
      bitwise_rshift(batch<uint32_t, A>(v), 16, avx{}); /* 16 most significant bits of v */
...
inline batch<uint32_t, A> fast_cast(batch<float, A> const& self, batch<uint32_t, A> const&,
  ...
  return _mm256_castps_si256(_mm256_blendv_ps(
      _mm256_castsi256_ps(_mm256_cvttps_epi32(self)),
      _mm256_xor_ps(
...
inline batch_bool<float, A> eq(batch_bool<float, A> const& self, batch_bool<float, A> const& other,
                               requires_arch<avx>) noexcept {
  return _mm256_castsi256_ps(detail::fwd_to_sse(
      [](__m128i s, __m128i o) noexcept {
...
inline batch_bool<double, A> eq(batch_bool<double, A> const& self,
                                batch_bool<double, A> const& other, requires_arch<avx>) noexcept {
  return _mm256_castsi256_pd(detail::fwd_to_sse(
      [](__m128i s, __m128i o) noexcept {
        return eq(batch_bool<int32_t, sse4_2>(s), batch_bool<int32_t, sse4_2>(o), sse4_2{});
      },
...
inline batch_bool<T, A> from_mask(batch_bool<T, A> const&, uint64_t mask,
  ...
  XSIMD_IF_CONSTEXPR(sizeof(T) == 1) {
    ...
    return _mm256_setr_epi32(lut32[mask & 0xF], lut32[(mask >> 4) & 0xF], lut32[(mask >> 8) & 0xF],
                             lut32[(mask >> 12) & 0xF], lut32[(mask >> 16) & 0xF],
                             lut32[(mask >> 20) & 0xF], lut32[(mask >> 24) & 0xF],
                             lut32[mask >> 28]);
  }
  else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
inline batch_bool<float, A> neq(batch_bool<float, A> const& self,
                                batch_bool<float, A> const& other, requires_arch<avx>) noexcept {
  return _mm256_xor_ps(self, other);
}
template <class A>
...
inline batch<float, A> select(batch_bool_constant<batch<float, A>, Values...> const&,
...
template <class A, bool... Values>
inline batch<double, A> select(batch_bool_constant<batch<double, A>, Values...> const&,
                               batch<double, A> const& true_br, batch<double, A> const& false_br,
                               requires_arch<avx>) noexcept {
...
template <class A, class... Values>
inline batch<double, A> set(batch<double, A> const&, requires_arch<avx>,
...
template <class A, class T,
          class = typename std::enable_if<std::is_integral<T>::value, void>::type>
inline batch<T, A> set(batch<T, A> const&, requires_arch<avx>, T v0, T v1, T v2, T v3, T v4, T v5,
                       T v6, T v7) noexcept {
...
template <class A, class T,
          class = typename std::enable_if<std::is_integral<T>::value, void>::type>
inline batch<T, A> set(batch<T, A> const&, requires_arch<avx>, T v0, T v1, T v2, T v3, T v4, T v5,
                       T v6, T v7, T v8, T v9, T v10, T v11, T v12, T v13, T v14, T v15, T v16,
                       T v17, T v18, T v19, T v20, T v21, T v22, T v23, T v24, T v25, T v26, T v27,
                       T v28, T v29, T v30, T v31) noexcept {
...
template <class A, class T, class... Values,
          class = typename std::enable_if<std::is_integral<T>::value, void>::type>
inline batch_bool<T, A> set(batch_bool<T, A> const&, requires_arch<avx>,
                            Values... values) noexcept {
  return set(batch<T, A>(), A{}, static_cast<T>(values ? -1LL : 0LL)...).data;
}

template <class A, class... Values>
inline batch_bool<float, A> set(batch_bool<float, A> const&, requires_arch<avx>,
                                Values... values) noexcept {
...
template <class A, class... Values>
inline batch_bool<double, A> set(batch_bool<double, A> const&, requires_arch<avx>,
                                 Values... values) noexcept {
...
inline batch<float, A> swizzle(batch<float, A> const& self,
                               batch_constant<batch<uint32_t, A>, V0, V1, V2, V3, V4, V5, V6, V7>,
                               requires_arch<avx>) noexcept {
  ...
  // normalize mask
  batch_constant<batch<uint32_t, A>, (V0 % 4), (V1 % 4), (V2 % 4), (V3 % 4), (V4 % 4), (V5 % 4),
                 (V6 % 4), (V7 % 4)>
      half_mask;
  ...
  batch_bool_constant<batch<uint32_t, A>, (V0 >= 4), (V1 >= 4), (V2 >= 4), (V3 >= 4), (V4 >= 4),
                      (V5 >= 4), (V6 >= 4), (V7 >= 4)>
      blend_mask;
...
inline batch<double, A> swizzle(batch<double, A> const& self,
                                batch_constant<batch<uint64_t, A>, V0, V1, V2, V3>,
                                requires_arch<avx>) noexcept {
  // duplicate low and high part of input
  ...
  // normalize mask
  batch_constant<batch<uint64_t, A>, (V0 % 2) * -1, (V1 % 2) * -1, (V2 % 2) * -1, (V3 % 2) * -1>
      half_mask;
...
inline batch<T, A> swizzle(
    batch<T, A> const& self,
    batch_constant<batch<uint32_t, A>, V0, V1, V2, V3, V4, V5, V6, V7> const& mask,
    requires_arch<avx>) noexcept {
  return bitwise_cast<batch<T, A>>(swizzle(bitwise_cast<batch<float, A>>(self), mask));
}
...
inline batch<T, A> swizzle(batch<T, A> const& self,
                           ...
                           requires_arch<avx>) noexcept {
  return bitwise_cast<batch<T, A>>(swizzle(bitwise_cast<batch<double, A>>(self), mask));
}
