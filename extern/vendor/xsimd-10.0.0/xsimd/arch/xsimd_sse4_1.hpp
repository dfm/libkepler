...
inline batch<uint32_t, A> fast_cast(batch<float, A> const& self, batch<uint32_t, A> const&,
  ...
  return _mm_castps_si128(_mm_blendv_ps(
      _mm_castsi128_ps(_mm_cvttps_epi32(self)),
      _mm_castsi128_ps(_mm_xor_si128(_mm_cvttps_epi32(_mm_sub_ps(self, _mm_set1_ps(1u << 31))),
...
inline batch<T, A> mul(batch<T, A> const& self, batch<T, A> const& other,
  ...
  XSIMD_IF_CONSTEXPR(sizeof(T) == 1) {
    ...
    return _mm_add_epi64(
        ...
        _mm_slli_epi64(
            _mm_add_epi64(_mm_mul_epu32(other, _mm_shuffle_epi32(self, _MM_SHUFFLE(2, 3, 0, 1))),
