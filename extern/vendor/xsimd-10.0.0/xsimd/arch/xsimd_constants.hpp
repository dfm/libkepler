...
XSIMD_DEFINE_CONSTANT(invlog10_2, 3.32192809488736234787031942949f,
...
struct minvalue_impl<float> {
  static float get_value() noexcept { return bit_cast<float>((uint32_t)0xff7fffff); }
};

template <>
struct minvalue_impl<double> {
  static double get_value() noexcept { return bit_cast<double>((uint64_t)0xffefffffffffffff); }
};
}  // namespace detail
