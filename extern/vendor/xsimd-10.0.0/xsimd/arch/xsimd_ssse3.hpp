...
inline batch<uint16_t, A> swizzle(
    ...
    batch_constant<batch<uint16_t, A>, V0, V1, V2, V3, V4, V5, V6, V7>,
    requires_arch<ssse3>) noexcept {
  constexpr batch_constant<batch<uint8_t, A>, 2 * V0, 2 * V0 + 1, 2 * V1, 2 * V1 + 1, 2 * V2,
                           2 * V2 + 1, 2 * V3, 2 * V3 + 1, 2 * V4, 2 * V4 + 1, 2 * V5, 2 * V5 + 1,
...
inline batch<int16_t, A> swizzle(
    ...
    batch_constant<batch<uint16_t, A>, V0, V1, V2, V3, V4, V5, V6, V7> mask,
    requires_arch<ssse3>) noexcept {
  return bitwise_cast<batch<int16_t, A>>(
      swizzle(bitwise_cast<batch<uint16_t, A>>(self), mask, ssse3{}));
}
...
inline batch<int8_t, A> swizzle(batch<int8_t, A> const& self,
  ...
  return bitwise_cast<batch<int8_t, A>>(
      swizzle(bitwise_cast<batch<uint8_t, A>>(self), mask, ssse3{}));
}
