...
inline batch_bool<T, A> from_mask(batch_bool<T, A> const&, uint64_t mask,
  ...
  for (size_t i = 0; i < batch_bool<T, A>::size; ++i) buffer[i] = mask & (1ull << i);
...
template <class A>
inline batch_bool<float, A> isfinite(batch<float, A> const& self,
                                     requires_arch<generic>) noexcept {
  return (self - self) == 0.f;
}
template <class A>
inline batch_bool<double, A> isfinite(batch<double, A> const& self,
                                      requires_arch<generic>) noexcept {
  return (self - self) == 0.;
}
...
// logical_and
template <class A, class T>
inline batch<T, A> logical_and(batch<T, A> const& self, batch<T, A> const& other,
                               requires_arch<generic>) noexcept {
...
// logical_or
template <class A, class T>
inline batch<T, A> logical_or(batch<T, A> const& self, batch<T, A> const& other,
                              requires_arch<generic>) noexcept {
  return detail::apply([](T x, T y) noexcept { return x || y; }, self, other);
}
