...
// batch_cast
template <class A, class T>
inline batch<T, A> batch_cast(batch<T, A> const& self, batch<T, A> const&,
...
inline batch<T_out, A> batch_cast(batch<T_in, A> const& self, batch<T_out, A> const& out,
                                  requires_arch<generic>, with_fast_conversion) noexcept {
  return fast_cast(self, out, A{});
}
template <class A, class T_out, class T_in>
inline batch<T_out, A> batch_cast(batch<T_in, A> const& self, batch<T_out, A> const&,
                                  requires_arch<generic>, with_slow_conversion) noexcept {
...
// bitwise_cast
template <class A, class T>
inline batch<T, A> bitwise_cast(batch<T, A> const& self, batch<T, A> const&,
                                requires_arch<generic>) noexcept {
  return self;
}
...
// clip
template <class A, class T>
inline batch<T, A> clip(batch<T, A> const& self, batch<T, A> const& lo, batch<T, A> const& hi,
                        requires_arch<generic>) noexcept {
...
inline batch<T, A> copysign(batch<T, A> const& self, batch<T, A> const& other,
                            requires_arch<generic>) noexcept {
...
inline batch<double, A> erfc(batch<double, A> const& self, requires_arch<generic>) noexcept {
  ...
  if (any(test1)) {
    ...
    if (all(test1)) return select(test0, batch_type(2.) - r1, r1);
  }
  auto test2 = x < lim2;
  auto test3 = test2 && !test1;
  batch_type ex = exp(-xx);
  if (any(test3)) {
    ...
    if (all(test1 || test3)) return select(test0, batch_type(2.) - r1, r1);
  }
...
struct estrin {
 ...
 private:
  inline B eval(const B& c0) noexcept { return c0; }

  inline B eval(const B& c0, const B& c1) noexcept { return fma(x, c1, c0); }
  ...
  inline B eval(const std::tuple<Args...>& tuple, const B& c0, const B& c1,
...
struct exp_reduction<double, A, exp_tag> : exp_reduction_base<batch<double, A>, exp_tag> {
  ...
  static inline batch_type reduce(const batch_type& a, batch_type& hi, batch_type& lo,
...
struct exp_reduction<double, A, exp10_tag> : exp_reduction_base<batch<double, A>, exp10_tag> {
  ...
  static inline batch_type approx(const batch_type& x) noexcept {
    ...
    batch_type x2 = px / (detail::horner1<batch_type, 0x40a03f37650df6e2ull, 0x4093e05eefd67782ull,
                                          0x405545fdce51ca08ull>(xx) -
                          px);
    return ++(x2 + x2);
  }

  static inline batch_type reduce(const batch_type& a, batch_type&, batch_type&,
                                  batch_type& x) noexcept {
    batch_type k = nearbyint(constants::invlog10_2<batch_type>() * a);
    x = fnma(k, constants::log10_2hi<batch_type>(), a);
  ...
  static inline batch_type finalize(const batch_type&, const batch_type& c, const batch_type&,
                                    const batch_type&) noexcept {
    return c;
  }
};
...
struct exp_reduction<double, A, exp2_tag> : exp_reduction_base<batch<double, A>, exp2_tag> {
  ...
  static inline batch_type finalize(const batch_type& x, const batch_type& c, const batch_type&,
                                    const batch_type&) noexcept {
...
template <class A, class T>
inline batch<std::complex<T>, A> exp(batch<std::complex<T>, A> const& self,
                                     requires_arch<generic>) noexcept {
...
static inline batch<float, A> expm1(const batch<float, A>& a) noexcept {
  ...
  using i_type = as_integer_t<batch_type>;
  i_type ik = to_int(k);
  batch_type two2mk = ::xsimd::bitwise_cast<batch_type>((constants::maxexponent<batch_type>() - ik)
...
inline batch<T, A> expm1(batch<T, A> const& self, requires_arch<generic>) noexcept {
  using batch_type = batch<T, A>;
  return select(self < constants::logeps<batch_type>(), batch_type(-1.),
                select(self > constants::maxlog<batch_type>(), constants::infinity<batch_type>(),
                       detail::expm1(self)));
}

template <class A, class T>
inline batch<std::complex<T>, A> expm1(const batch<std::complex<T>, A>& z,
                                       requires_arch<generic>) noexcept {
  using batch_type = batch<std::complex<T>, A>;
...
// polar
template <class A, class T>
inline batch<std::complex<T>, A> polar(const batch<T, A>& r, const batch<T, A>& theta,
                                       requires_arch<generic>) noexcept {
...
// fdim
template <class A, class T>
inline batch<T, A> fdim(batch<T, A> const& self, batch<T, A> const& other,
                        requires_arch<generic>) noexcept {
...
// fmod
template <class A, class T>
inline batch<T, A> fmod(batch<T, A> const& self, batch<T, A> const& other,
                        requires_arch<generic>) noexcept {
 ...
 */
template <class A, class T>
inline batch<T, A> frexp(const batch<T, A>& self, batch<as_integer_t<T>, A>& exp,
                         requires_arch<generic>) noexcept {
  using batch_type = batch<T, A>;
  ...
  return select((self != batch_type(0.)),
                x | ::xsimd::bitwise_cast<batch_type>(constants::mask2frexp<batch_type>()),
                batch_type(0.));
}
...
// hypot
template <class A, class T>
inline batch<T, A> hypot(batch<T, A> const& self, batch<T, A> const& other,
                         requires_arch<generic>) noexcept {
 ...
 */
template <class A, class T>
inline batch<T, A> ldexp(const batch<T, A>& self, const batch<as_integer_t<T>, A>& other,
                         requires_arch<generic>) noexcept {
  using batch_type = batch<T, A>;
 ...
 private:
  ...
  static inline batch_type other(const batch_type& x) noexcept {
    ...
    if (any(xlt650)) {
      ...
      if (all(xlt650)) return fma(r0x, p, r0s * log(abs(r0z)));
    }
...
struct lgamma_impl<batch<double, A>> {
  ...
  static inline batch_type compute(const batch_type& a) noexcept {
    ...
    if (any(test)) {
      r = large_negative(q);
      if (all(test)) return select(inf_result, constants::nan<batch_type>(), r);
    }
...
inline batch<double, A> log(batch<double, A> const& self, requires_arch<generic>) noexcept {
  ...
  x = ::xsimd::bitwise_cast<batch_type>(hx << 32 |
                                        (i_type(0xffffffff) & ::xsimd::bitwise_cast<i_type>(x)));
  ...
  batch_type t1 =
      w *
      detail::horner<batch_type, 0x3fd999999997fa04ll, 0x3fcc71c51d8e78afll, 0x3fc39a09d078c69fll>(
          w);
...
template <class A, class T>
inline batch<std::complex<T>, A> log(const batch<std::complex<T>, A>& z,
                                     requires_arch<generic>) noexcept {
...
inline batch<T, A> logN_complex_impl(const batch<T, A>& z,
...
template <class A, class T>
inline batch<std::complex<T>, A> log2(batch<std::complex<T>, A> const& self,
                                      requires_arch<generic>) noexcept {
...
template <class A, class T>
inline batch<std::complex<T>, A> log10(const batch<std::complex<T>, A>& z,
                                       requires_arch<generic>) noexcept {
...
inline batch<double, A> log1p(batch<double, A> const& self, requires_arch<generic>) noexcept {
  ...
  batch_type f = ::xsimd::bitwise_cast<batch_type>(
      (hu << 32) | (i_type(0xffffffff) & ::xsimd::bitwise_cast<i_type>(uf)));
  f = --f;
  batch_type hfsq = batch_type(0.5) * f * f;
  batch_type s = f / (batch_type(2.) + f);
  batch_type z = s * s;
...
template <class A, class T>
inline batch<std::complex<T>, A> log1p(batch<std::complex<T>, A> const& self,
                                       requires_arch<generic>) noexcept {
  using batch_type = batch<std::complex<T>, A>;
  using real_batch = typename batch_type::real_batch;
  batch_type u = 1 + self;
...
template <class A, class T,
          class = typename std::enable_if<std::is_integral<T>::value, void>::type>
inline batch<T, A> mod(batch<T, A> const& self, batch<T, A> const& other,
                       requires_arch<generic>) noexcept {
...
// nearbyint_as_int
template <class A>
inline batch<as_integer_t<float>, A> nearbyint_as_int(batch<float, A> const& self,
                                                      requires_arch<generic>) noexcept {
...
template <class A>
inline batch<as_integer_t<double>, A> nearbyint_as_int(batch<double, A> const& self,
                                                       requires_arch<generic>) noexcept {
...
struct nextafter_kernel {
  ...
  static inline batch_type next(batch_type const& b) noexcept { return b; }

  static inline batch_type prev(batch_type const& b) noexcept { return b; }
};
...
struct nextafter_kernel<T, A, false> {
  ...
  static inline batch_type next(const batch_type& b) noexcept {
    batch_type n =
        ::xsimd::bitwise_cast<batch_type>(::xsimd::bitwise_cast<int_batch>(b) + int_type(1));
...
template <class A, class T>
inline batch<T, A> nextafter(batch<T, A> const& from, batch<T, A> const& to,
                             requires_arch<generic>) noexcept {
 ...
 */
template <class A, class T>
inline batch<T, A> pow(batch<T, A> const& self, batch<T, A> const& other,
                       requires_arch<generic>) noexcept {
  using batch_type = batch<T, A>;
...
template <class A, class T>
inline batch<std::complex<T>, A> pow(const batch<std::complex<T>, A>& a,
...
inline batch<T, A> reciprocal(batch<T, A> const& self, requires_arch<generic>) noexcept {
  using batch_type = batch<T, A>;
...
// reduce_add
template <class A, class T>
inline std::complex<T> reduce_add(batch<std::complex<T>, A> const& self,
                                  requires_arch<generic>) noexcept {
...
struct split_high {
  static constexpr T get(T i, T) { return i >= N ? (i % 2) : i + N; }
};
...
inline T reduce(Op op, batch<T, A> const& self, std::integral_constant<unsigned, Lvl>) noexcept {
  ...
  batch<T, A> split =
      swizzle(self, make_batch_constant<batch<index_type, A>, split_high<index_type, Lvl / 2>>());
...
inline T reduce_max(batch<T, A> const& self, requires_arch<generic>) noexcept {
  return detail::reduce([](batch<T, A> const& x, batch<T, A> const& y) { return max(x, y); }, self,
...
inline T reduce_min(batch<T, A> const& self, requires_arch<generic>) noexcept {
  return detail::reduce([](batch<T, A> const& x, batch<T, A> const& y) { return min(x, y); }, self,
                        std::integral_constant<unsigned, batch<T, A>::size>());
}

// remainder
template <class A>
inline batch<float, A> remainder(batch<float, A> const& self, batch<float, A> const& other,
                                 requires_arch<generic>) noexcept {
  return fnma(nearbyint(self / other), other, self);
}
template <class A>
inline batch<double, A> remainder(batch<double, A> const& self, batch<double, A> const& other,
                                  requires_arch<generic>) noexcept {
  return fnma(nearbyint(self / other), other, self);
}
template <class A, class T,
          class = typename std::enable_if<std::is_integral<T>::value, void>::type>
inline batch<T, A> remainder(batch<T, A> const& self, batch<T, A> const& other,
                             requires_arch<generic>) noexcept {
  auto mod = self % other;
...
// select
template <class A, class T>
inline batch<std::complex<T>, A> select(batch_bool<T, A> const& cond,
...
template <class A, class T>
inline batch<std::complex<T>, A> sign(const batch<std::complex<T>, A>& z,
                                      requires_arch<generic>) noexcept {
  using batch_type = batch<std::complex<T>, A>;
  using real_batch = typename batch_type::real_batch;
  auto rz = z.real();
...
// sqrt
template <class A, class T>
inline batch<std::complex<T>, A> sqrt(batch<std::complex<T>, A> const& z,
                                      requires_arch<generic>) noexcept {
...
inline batch<T, A> tgamma(batch<T, A> const& self, requires_arch<generic>) noexcept {
  ...
  if (any(test)) {
    ...
    if (all(test)) return select(nan_result, constants::nan<batch_type>(), r);
  }
