 ...
 */
template <class T, class A>
inline auto bitwise_and(batch_bool<T, A> const& x, batch_bool<T, A> const& y) noexcept
    -> decltype(x & y) {
  detail::static_check_supported_config<T, A>();
  return x & y;
}
 ...
 */
template <class T, class A>
inline batch_bool<T, A> bitwise_andnot(batch_bool<T, A> const& x,
 ...
 */
template <class T, class A>
inline auto bitwise_or(batch_bool<T, A> const& x, batch_bool<T, A> const& y) noexcept
    -> decltype(x | y) {
  detail::static_check_supported_config<T, A>();
  return x | y;
}
 ...
 */
template <class T, class A>
inline auto bitwise_xor(batch_bool<T, A> const& x, batch_bool<T, A> const& y) noexcept
    -> decltype(x ^ y) {
  detail::static_check_supported_config<T, A>();
  return x ^ y;
}
 ...
 */
template <class T, class A>
inline batch<T, A> clip(batch<T, A> const& x, batch<T, A> const& lo,
 ...
 */
template <class T, class A>
inline batch<T, A> extract_pair(batch<T, A> const& x, batch<T, A> const& y,
 ...
 */
template <class T, class A>
inline batch<T, A> fnma(batch<T, A> const& x, batch<T, A> const& y,
 ...
 */
template <class T, class A>
inline batch<T, A> fnms(batch<T, A> const& x, batch<T, A> const& y,
                        batch<T, A> const& z) noexcept {
  detail::static_check_supported_config<T, A>();
...
template <class To, class A = default_arch, class From>
inline simd_return_type<std::complex<From>, To, A> load_as(std::complex<From> const* ptr,
...
inline simd_return_type<xtl::xcomplex<From, From, i3ec>, To, A> load_as(
    xtl::xcomplex<From, From, i3ec> const* ptr, aligned_mode) noexcept {
  detail::static_check_supported_config<To, A>();
  detail::static_check_supported_config<From, A>();
...
template <class To, class A = default_arch, class From, bool i3ec>
inline simd_return_type<xtl::xcomplex<From, From, i3ec>, To, A> load_as(
    xtl::xcomplex<From, From, i3ec> const* ptr, unaligned_mode) noexcept {
  detail::static_check_supported_config<To, A>();
  detail::static_check_supported_config<From, A>();
 ...
 */
template <class T, class A>
inline batch<as_integer_t<T>, A> nearbyint_as_int(batch<T, A> const& x) noexcept {
  detail::static_check_supported_config<T, A>();
 ...
 */
template <class T, class A>
inline complex_batch_type_t<batch<T, A>> polar(batch<T, A> const& r,
                                               batch<T, A> const& theta = batch<T, A>{}) noexcept {
  detail::static_check_supported_config<T, A>();
...
inline batch<T, A> select(batch_bool_constant<batch<T, A>, Values...> const& cond,
...
template <class To, class A = default_arch, class From>
inline void store_as(std::complex<To>* dst, batch<std::complex<From>, A> const& src,
                     aligned_mode) noexcept {
...
#ifdef XSIMD_ENABLE_XTL_COMPLEX
template <class To, class A = default_arch, class From, bool i3ec>
inline void store_as(xtl::xcomplex<To, To, i3ec>* dst, batch<std::complex<From>, A> const& src,
                     aligned_mode) noexcept {
...
inline typename std::enable_if<std::is_arithmetic<T>::value, batch<T, A>>::type swizzle(
    batch<T, A> const& x, batch_constant<batch<Vt, A>, Values...> mask) noexcept {
  static_assert(sizeof(T) == sizeof(Vt), "consistent mask");
  detail::static_check_supported_config<T, A>();
  return kernel::swizzle<A>(x, mask, A{});
}
template <class T, class A, class Vt, Vt... Values>
inline batch<std::complex<T>, A> swizzle(batch<std::complex<T>, A> const& x,
                                         batch_constant<batch<Vt, A>, Values...> mask) noexcept {
  static_assert(sizeof(T) == sizeof(Vt), "consistent mask");
  detail::static_check_supported_config<T, A>();
  return kernel::swizzle<A>(x, mask, A{});
}
...
inline std::ostream& operator<<(std::ostream& o, batch<T, A> const& x) noexcept {
  ...
  o << '(';
  for (std::size_t i = 0; i < size - 1; ++i) o << buffer[i] << ", ";
