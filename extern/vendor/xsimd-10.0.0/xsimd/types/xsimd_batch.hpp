...
class batch<std::complex<T>, A> {
 public:
  ...
  // memory operators
  XSIMD_NO_DISCARD static batch load_aligned(const T* real_src,
                                             const T* imag_src = nullptr) noexcept;
  XSIMD_NO_DISCARD static batch load_unaligned(const T* real_src,
                                               const T* imag_src = nullptr) noexcept;
 ...
 */
template <class T, class A>
inline batch<T, A>::batch(batch_bool<T, A> const& b) noexcept : batch(kernel::from_bool(b, A{})) {}
 ...
 */
template <class T, class A>
inline batch<T, A>::batch(register_type reg) noexcept : types::simd_register<T, A>({reg}) {
  detail::static_check_supported_config<T, A>();
}
...
template <class T, class A>
inline batch<T, A>& types::integral_only_operators<T, A>::operator%=(
...
template <class T, class A>
inline batch<T, A>& kernel::integral_only_operators<T, A>::operator>>=(
    batch<T, A> const& other) noexcept {
  ::xsimd::detail::static_check_supported_config<T, A>();
  return *static_cast<batch<T, A>*>(this) =
...
inline batch_bool<T, A> batch_bool<T, A>::load_aligned(bool const* mem) noexcept {
  ...
  for (std::size_t i = 0; i < size; ++i) buffer[i] = mem[i] ? 1 : 0;
...
template <class T, class A>
inline batch_bool<T, A> batch_bool<T, A>::operator==(
    batch_bool<T, A> const& other) const noexcept {
...
template <class T, class A>
template <class U, class... V, size_t I, size_t... Is>
inline auto batch_bool<T, A>::make_register(detail::index_sequence<I, Is...>, U u, V... v) noexcept
    -> register_type {
...
template <class T, class A>
template <class... V>
inline auto batch_bool<T, A>::make_register(detail::index_sequence<>, V... v) noexcept
    -> register_type {
...
template <class T, class A>
inline batch<std::complex<T>, A>::batch(T val) noexcept : m_real(val), m_imag(0) {}
...
template <class T, class A>
template <class U>
XSIMD_NO_DISCARD inline batch<std::complex<T>, A> batch<std::complex<T>, A>::broadcast(
    U val) noexcept {
...
template <class T, class A>
inline batch<std::complex<T>, A> batch<std::complex<T>, A>::load_aligned(
    const T* real_src, const T* imag_src) noexcept {
  return {batch<T, A>::load_aligned(real_src),
          imag_src ? batch<T, A>::load_aligned(imag_src) : batch<T, A>(0)};
}
template <class T, class A>
inline batch<std::complex<T>, A> batch<std::complex<T>, A>::load_unaligned(
    const T* real_src, const T* imag_src) noexcept {
...
template <class T, class A>
template <class U>
inline batch<std::complex<T>, A> batch<std::complex<T>, A>::load(U const* mem,
...
template <class T, class A>
template <bool i3ec, class... Ts>
inline batch<std::complex<T>, A>::batch(xtl::xcomplex<T, T, i3ec> val0,
...
template <bool i3ec>
inline batch<std::complex<T>, A> batch<std::complex<T>, A>::load_aligned(
    const xtl::xcomplex<T, T, i3ec>* src) noexcept {
...
template <class T, class A>
template <bool i3ec>
inline void batch<std::complex<T>, A>::store_aligned(
    xtl::xcomplex<T, T, i3ec>* dst) const noexcept {
  store_aligned(reinterpret_cast<std::complex<T>*>(dst));
}

template <class T, class A>
template <bool i3ec>
inline void batch<std::complex<T>, A>::store_unaligned(
    xtl::xcomplex<T, T, i3ec>* dst) const noexcept {
...
template <typename T, class Arch,
          bool BatchExists = xsimd::types::has_simd_register<T, Arch>::value>
struct batch_trait;
